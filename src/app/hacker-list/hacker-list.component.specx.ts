...
import { Hacker } from 'app/core/hacker.model';
...
describe('HackerListComponent: Solution', () => {
  ...
  describe('initial display', () => {
    ...
    it('makes a call to api.getHackers', () => {
      spyOn(api, 'getHackers').and.returnValue(Promise.resolve(<Hacker[]>{}));
    ...
    it('sets initial data (using async)', async(() => {
      spyOn(api, 'getHackers').and.returnValue(Promise.resolve(<Hacker[]>mockHackers));

      fixture.detectChanges();

      fixture.whenStable()
        .then(() => {
          expect(component.hackers).toEqual(<Hacker[]>mockHackers);
        });
